...
class AdminMessage(db.Model):
    ...
    sent_by = db.Column(db.String(200), default='admin')
...
def order_to_dict(o):
    ...
    if o.payments:
        last_pay = sorted(
            o.payments, key=lambda p: p.paid_at or datetime.min)[-1]
        last_payment_type = last_pay.payment_type or ''

    # Count messages
    msgs = AdminMessage.query.filter_by(order_id=o.id).order_by(
        AdminMessage.sent_at.asc()).all()
    messages_list = [{
        'id': m.id,
        'message': m.message,
        'sent_by': m.sent_by,
...
def create_order():
    ...
    if sub_id:
        ...
        if existing:
            print(
                f"[ORDER] Dedup hit — returning existing {existing.order_id} for submission_id={sub_id}")
    ...
    if cust:
        ...
        if recent_dup:
            print(
                f"[ORDER] Time-based dedup — returning existing {recent_dup.order_id} for {email}")
    ...
    try:
        ...
        if raw:
            ...
            evt_date = date.fromisoformat(raw[:10])
    except Exception:
        pass
    ...
    if capacity >= 3000:
        # Date+slot already has 3000 or more guests booked → wait for admin approval
        auto_status = 'pending'
    else:
        # Slot is free → auto-approve immediately
...
def export_excel():
    ...
    if date_str:
        try:
            ...
            orders = Order.query.filter(Order.event_date == fd).order_by(
...
def daily_count():
    ...
    confirmed = Order.query.filter(
        Order.event_date == d, Order.status == 'confirmed').count()
    pending = Order.query.filter(
        Order.event_date == d, Order.status == 'pending').count()
...
def update_order_email(oid):
    ...
    try_email(send_order_update, o, o.customer,
              f"Update on Order #{o.order_id}", msg)
...
def get_order_messages(oid):
    ...
    msgs = AdminMessage.query.filter_by(order_id=oid).order_by(
        AdminMessage.sent_at.asc()).all()
    return jsonify([{
        'id': m.id,
        'message': m.message,
...
def send_order_message(oid):
    ...
    from services.email_service import send_order_update
    try_email(send_order_update, o, o.customer,
              f"Message from MenuMatrix — Order #{o.order_id}", msg)
...
def get_capacity():
    orders = Order.query.filter(
        Order.status.in_(['approved', 'confirmed'])).all()
    capacity_map = {}
    for o in orders:
        if not o.event_date:
            continue
...
def create_payment():
    ...
    if o.status == 'confirmed':
        existing_paid = sum(float(p.amount_paid or 0)
    ...
    previously_paid = sum(float(p.amount_paid or 0)
                          for p in (o.payments or []))
    ...
    db.session.commit()
    print(
        f"[PAYMENT] Order {o.order_id} confirmed. ThisPayment=Rs.{paid} TotalPaid=Rs.{total_paid} Balance=Rs.{balance} TXN={txn}")
...
def my_orders():
    ...
    if not cust:
        return jsonify([])
    orders = Order.query.filter_by(customer_id=cust.id).order_by(
        Order.created_at.desc()).all()
    return jsonify([order_to_dict(o) for o in orders])
